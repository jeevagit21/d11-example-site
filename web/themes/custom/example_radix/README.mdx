...
example_radix theme uses [Webpack](https://webpack.js.org) to compile and
bundle SASS and JS.
...
Go to the root of example_radix theme and run the following commands:
`nvm use`
...
`drush en example_radix -y`
`drush config-set system.theme default example_radix -y`
